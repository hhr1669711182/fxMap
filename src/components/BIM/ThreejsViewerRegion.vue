...
// const clockTruck = new THREE.Timer()
...
const initThree = async () => {
  ...
  camera.position.set(3, 6, 9); // 俯瞰视角  
  // 创建渲染器
...
async function loadFireTruck() {
  ...
  const localCoord = lonLatToLocalCoord(113.54776032, 22.22632892, true) 
...
function drawTruckPath(){
  const coordinates= [ [ 113.54776032, 22.22632892 ], 
   ...
   const shapePoints = [];
    ...
    createPath2(shapePoints)
    createPath(shapePoints) 

    // let aa=insertIntermediatePoints([ 113.54776032, 22.22632892 ], [ 113.54818221, 22.22296648 ])
    // console.log("hello", aa)
  //   const coordinates2= [ [113.547886886, 22.22532018],    
  //  [113.547971265, 22.2246477] ]   

  //  const shapePoints2 = [];
  //   coordinates2.forEach(([lon, lat]) => {
  //     const localCoord = lonLatToLocalCoord(lon, lat, true);
  //     shapePoints2.push(localCoord);
  //   });  
    //createPath(shapePoints2, 0xff0000)  
}

function insertIntermediatePoints(pointA, pointB) {
  const [lon1, lat1] = pointA
  const [lon2, lat2] = pointB

  // 30% 位置
  const lon30 = lon1 + (lon2 - lon1) * 0.3
  const lat30 = lat1 + (lat2 - lat1) * 0.3
  
  // 50% 位置
  const lon50 = lon1 + (lon2 - lon1) * 0.5
  const lat50 = lat1 + (lat2 - lat1) * 0.5
...
function createPath2(pathPoints) {
...
// 加宽路径线
function createPath(pathPoints, color= 0x00ff00) {  
  // 把点拆成 xyz 数组
  const positions = []
...
function moveFireTruckAlongPath(clockChange) {     
  ...
  fireTruck.rotation.y += -Math.PI / 2
    ...
    //const cameraOffset = new THREE.Vector3(0, 4, -8); // 相机在车后上方
...
function createExtrudeWall(points, wallHeight, wallThickness, addH) {
  ...
  const material = new THREE.MeshLambertMaterial({ color: 0xB7C780, transparent: true, opacity: 0.9, side: THREE.DoubleSide })
  const wall = new THREE.Mesh(geometry, material)
  wall.rotation.x = -Math.PI / 2  // 平放
...
const animate = () => {
  ...
  const clockChange = Math.min((now - lastTime) / 1000, 0.1); // 防时间跳跃
  lastTime = now;
 
  animateId = requestAnimationFrame(animate);
  //clockTruck.update()
  //let clockChange = clockTruck.getDelta()
  if (mixer) {
    mixer.update(clockChange);
  }
  moveFireTruckAlongPath(clockChange)
